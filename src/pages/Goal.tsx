...
import { Trophy, Coins, CheckCircle2, PlayCircle, Loader2 } from 'lucide-react';
...
export function Goal() {
  ...
  const renderMatch = (match: any) => {
    ...
    return (
      <div key={match.id} className="bg-[#151518] rounded-2xl p-4 border border-white/5 relative overflow-hidden shadow-lg">
        {displayStatus === 'live' && (
        ...
        <div className="flex items-center justify-between relative z-10 mb-4">
          <div className="flex flex-col items-center gap-2 flex-[2]">
            <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 p-2 relative overflow-hidden">
          ...
          <div className="flex flex-col items-center px-2 flex-1 justify-center relative">
             <div className="flex items-center gap-2 mb-1">
               <span className="text-xl font-bold bg-white/10 px-3 py-1 rounded text-white">{match.teamA.score ?? 0}</span>
               <span className="text-sm font-bold text-gray-500">-</span>
               <span className="text-xl font-bold bg-white/10 px-3 py-1 rounded text-white">{match.teamB.score ?? 0}</span>
             </div>
             <div className="font-black text-xs text-white/20 italic absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none -z-10 text-3xl">VS</div>
          </div>
        ...
        <div className="border-t border-white/5 pt-4 mt-2">
          {pred ? (
            <div className="flex flex-col items-center justify-center p-3 bg-black/20 rounded-xl border border-white/5">
              ...
              <div className="text-xs font-mono mt-1 text-[#FFD700] flex items-center gap-1">
                 Bet: {pred.betAmount} <Coins size={10} />
              </div>
              ...
              {canCollect && (
                 <button 
                   ...
                   className="mt-3 w-full py-2.5 rounded-lg bg-green-500 hover:bg-green-400 text-black font-bold active:scale-95 transition-all flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(34,197,94,0.3)]"
                 >
                   <PlayCircle size={16} /> Collect {winAmount} <Coins size={14} />
                 </button>
              )}
              {pred.collected && (
                 <div className="mt-3 w-full py-2 text-center rounded-lg bg-green-500/10 text-green-400 font-bold border border-green-500/20 text-xs flex items-center justify-center gap-1.5 uppercase">
                   <CheckCircle2 size={14} /> Claimed {winAmount}
                 </div>
              )}
              {isLoss && (
                 <div className="mt-3 w-full py-2 text-center rounded-lg bg-red-500/10 text-red-400 font-bold border border-red-500/20 text-xs uppercase tracking-wider">
                   Incorrect Prediction
                 </div>
              )}
              {displayStatus === 'live' && !match.winner && (
                 <div className="mt-3 text-xs text-yellow-400 font-bold animate-pulse">Match in progress...</div>
              )}
              {displayStatus === 'completed_pending_result' && !match.winner && (
                 <div className="mt-3 text-xs text-gray-400 italic">Match finished, awaiting final results...</div>
              )}
              {displayStatus === 'upcoming' && (
                 <div className="mt-3 text-xs text-gray-500 italic">Awaiting match start...</div>
              )}
            </div>
          ) : displayStatus === 'upcoming' ? (
            selectedMatch === match.id ? (
              <div className="space-y-3 animation-fade-in">
                <h4 className="text-center font-bold text-sm">Choose Winner</h4>
                <div className="flex gap-2">
                   <button onClick={() => setSelectedTeam('A')} className={`flex-1 py-3 rounded-xl border font-bold text-sm transition-all ${selectedTeam === 'A' ? 'bg-[#00f3ff]/20 border-[#00f3ff] text-[#00f3ff]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                     {match.teamA.name}
                   </button>
                   <button onClick={() => setSelectedTeam('draw')} className={`flex-1 py-3 rounded-xl border font-bold text-sm transition-all ${selectedTeam === 'draw' ? 'bg-yellow-500/20 border-yellow-500 text-yellow-500' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                     Draw
                   </button>
                   <button onClick={() => setSelectedTeam('B')} className={`flex-1 py-3 rounded-xl border font-bold text-sm transition-all ${selectedTeam === 'B' ? 'bg-[#00f3ff]/20 border-[#00f3ff] text-[#00f3ff]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                     {match.teamB.name}
                   </button>
                </div>
                ...
                <div className="flex justify-between items-center gap-3">
                   <div className="flex-1">
                     <input 
                       ...
                       placeholder="Amount" 
                       className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-sm text-white font-mono focus:outline-none focus:border-[#00f3ff]"
                     />
                   </div>
                   <button 
                     onClick={() => handlePredict(match.id)}
                     disabled={!selectedTeam || isProcessing || balance < betAmount}
                     className="flex-[2] py-2.5 rounded-lg bg-[#00f3ff] text-black font-bold active:scale-95 transition-all text-sm shadow-[0_0_10px_rgba(0,243,255,0.2)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                   >
                     {isProcessing ? <Loader2 size={16} className="animate-spin" /> : <>Confirm Prediction</>}
                   </button>
                </div>
              ...
              <button 
                ...
                className="w-full py-3 rounded-xl bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 text-green-400 font-bold active:scale-95 transition-all hover:bg-green-500/30 flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(34,197,94,0.1)]"
              >
                Place Prediction
              </button>
            )
          ) : (
             <div className="w-full py-3 rounded-xl bg-white/5 border border-white/10 text-gray-500 font-bold text-center text-sm uppercase tracking-wider">
               Betting Closed
             </div>
          )}
        </div>
  ...
  return (
    <div className="flex flex-col w-full h-full bg-[#0a0a0c] text-white relative overflow-hidden pb-24">
      ...
      <div className="flex-1 overflow-y-auto w-full px-4 pt-6 mt-safe-top mt-10 custom-scroll relative z-10">
        ...
        <div className="flex bg-black/50 p-1.5 rounded-2xl mb-6 border border-white/5 relative z-10 w-full max-w-md mx-auto shadow-inner">
          <button 
            onClick={() => {
               const twa = (window as any).Telegram?.WebApp;
