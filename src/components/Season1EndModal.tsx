...
export function Season1EndModal({ onClose }: Props) {
  ...
  const calculationSteps = [
    { label: "Season 1 Ended - Analyzing Data...", duration: 1500 },
      ...
      const processStep = () => {
        if (currentStep >= calculationSteps.length) {
          if (hasClaimedSeason1) {
            ...
            const base = 5000;
            const balanceBonus = Math.floor(balance / 1000000); // More generous for Season 1 end
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      console.error("Season 1 withdrawal failed", e);
      const twa = (window as any).Telegram?.WebApp;
      if (twa?.showAlert) {
        ...
        {step === 'calculating' ? (
          ...
          <div className="flex-1 flex flex-col w-full max-w-md mx-auto pt-2 sm:pt-6 pb-2 justify-between h-full max-h-[100dvh]">
            <div className="flex flex-col items-center flex-shrink-0">
              ...
              <h2 className="text-xl sm:text-3xl font-black text-white mb-1 sm:mb-2 tracking-tight text-center">Season 1 Ended</h2>
              <p className="text-white/50 text-center text-xs sm:text-sm mb-3 sm:mb-8 px-4 leading-relaxed max-w-[280px]">Based on your Season 1 activity, you have earned $PLUSH tokens!</p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center bg-[#1c1c1e] border-2 border-white/5 shadow-2xl rounded-[24px] sm:rounded-[32px] w-full py-6 sm:py-10 px-5 relative overflow-hidden flex-shrink min-h-[120px] sm:min-h-[200px] my-auto">
               ...
               <div className="flex flex-col items-center justify-center gap-1 sm:gap-2 relative z-10 w-full">
                  <span className="text-white/50 text-[10px] sm:text-sm font-bold uppercase tracking-[0.2em] mb-1 sm:mb-2">Total Season 1 Reward</span>
