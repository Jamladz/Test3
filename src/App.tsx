...
export default function App() {
  ...
  const [adTasks, setAdTasks] = useState([
    { id: 't1', title: 'Join CryptoX Channel', reward: 500, link: 'https://t.me/toncoin' },
    { id: 't2', title: 'Follow TON Updates', reward: 300, link: 'https://t.me/toncoin' },
    { id: 't3', title: 'Like Pinned Post', reward: 200, link: 'https://t.me/toncoin' },
  ]);
  ...
  const handlePostAd = async () => {
    ...
    const cost = Number(((Number(adImpressions) / 1200) * 1).toFixed(2));
      ...
      <header className="shrink-0 flex justify-between items-center px-4 py-3 bg-slate-800 rounded-b-[24px] shadow-[0_4px_16px_rgba(0,0,0,0.3)] z-40 relative">
        <div className="flex items-center space-x-3 rtl:space-x-reverse">
          ...
          <div className="flex flex-col">
             <span className="font-bold text-sm leading-tight text-white mb-0.5">{userName}</span>
        ...
        <div className="flex items-center space-x-2 rtl:space-x-reverse">
          <select 
            ...
            className="bg-slate-700 text-xs font-bold border-none rounded-xl px-2 py-1.5 outline-none focus:ring-2 focus:ring-cyan-500 transition-shadow appearance-none cursor-pointer"
          >
            <option value="en">EN</option>
            <option value="ru">RU</option>
            <option value="ar">AR</option>
          </select>
          <TonConnectButton className="!scale-[0.70] origin-right rtl:origin-left" />
        </div>
      </header>
        ...
        {activeTab === 'home' && (
          <motion.div initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }} className="flex flex-col space-y-4 h-full">
            ...
            <button 
              ...
              className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-extrabold py-3.5 rounded-2xl border-b-[4px] border-emerald-800 shadow-lg active:border-b-0 active:translate-y-[4px] transition-all flex items-center justify-center text-sm"
            >
              ...
              >
                ...
                {adImpressions && Number(adImpressions) >= 1200 && (
                  <span className="ml-2 flex items-center rtl:mr-2 rtl:ml-0 bg-white/20 px-2 py-0.5 rounded-lg text-xs">
                    {Number(((Number(adImpressions) / 1200) * 1).toFixed(2))} <TonImage className="w-3.5 h-3.5 ml-1 rtl:mr-1 rtl:ml-0" />
                  </span>
                )}
              </button>
            </div>
